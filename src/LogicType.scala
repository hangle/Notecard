/* date:   Nov 2, 2012
	...
	A condition string, such as:
			((1)=(1)and(2)=(2))or((3)=...) )
	grenerates a Logic list whose types are:
...
object LogicType {
	...
	//	override def toString="left="+left+" op="+operator+" right="+right
	...
	//	println("LogicType:   left="+leftValue+"  right="+rightValue)
		   ...
		   op match {
			case "=" =>
	//			println("LogicType case=:  left="+leftValue+"   right="+rightValue)
				val xxx=if(leftValue==rightValue) true; else false
				xxx
			case "<>" => 
				//println("Relation: "+leftValue+"<>"+rightValue);
			...
			case _=>
					println("Relation: unknown  operator:["+operator+"] "); 
					false
				}
			}	
		...
		def	matchMultipleStringsToTarget(leftValue:String, rightValue:String):Boolean={
			...
			//		println("LogicType match   array=")
				array.foreach(println)
			//		println("LogicType match   leftValue="+leftValue)
			//		println("LogicType array.contains(leftValue)="+array.contains(leftValue) )
		...
		def	qualifyValue(tag:String, value:String)= {
			...
			while(! tagString.isEmpty) {
				...
				e match {
						case "nc" =>   // no case 
						//	println("LogicType 'nc' before valueString="+valueString)
							valueString=valueString.toLowerCase
						//	println("LogicType 'nc' after valueString="+valueString)
						//	valueString
	...
	//	println("LogicTokener:  extract():  expr=:|"+expr+"|")
