...
//----------------------------------------
	...
	def startCardSet(notePanel:JPanel, lock:AnyRef, buttonSet:ButtonSet, statusLine:StatusLine)={ 
		...
		println("CardSet start to execute CardCommandAndDisplay")
		...
		executeCardCommandsAndDisplay(notePanel, 
										  ...
										  statusLine, 
										  listenerArray) 
						println("CardSet  return from executeCardCommandAsnDispl;ay")

				// ------------Card Set has been displayed--------------
				// arm Buttons and enter 'wait' state. 
				// Button press releases 'wait' state (ButtonSet.start() ):"
			//println("CardSet  isNoInputFields="+inputFocus.isNoInputFields)
...
//				println("CardSet turnOnPriorButton")
				}
						...
						//	println("CardSet isFirstChildFalse="+buttonSet.isFirstChildFalse)
		...
		println("CArdSet clear and remove fields")
		}
	...
	def executeCardCommandsAndDisplay(notePanel:JPanel,
							...
							// Create default line size and row spacing
							// by instantiating /  RowPosition(new Font("Serif", 0, size)) 
//			val rowPosition=initializeRowPosition(18) //***skip*** see LabelPixelHeight.java						
	...
	def iterateCardSetChildren(rowPosition:RowPosition, 
		...
		while(iterate) 
							...
							// resetBackupButton= backupButton=false //set false in Notecard after executing
											  // prior Card.
	...
	def whatToDo(   groupResolve: GroupResolve, 
		...
		groupResolve.actionToTake match{
			case  "do"  =>   // test was successful so execute to next 'g' or end of Card set
				iterateCardSetChildren(rowPosition, notePanel, lock, inputFocus, indexer, statusLine, listenerArray)
			case  "skip" =>  // test was false, so skip to next 'g' or to end of Card set
				//println("CardSet: case skip")
				...
				val groupNode=iterateToNextGroup(groupResolve, 
												 ...
												 statusLine, 
												 listenerArray)
				if(groupNode==null) println("CardSet:  groupNode is null"); else println("FRameNode groupNode is NOT null")
				if(groupNode!=null) { 
			...
			case  "done"=> 
				//println("CardSet actionToTake:  'done'  no else and no condition")
			}
		}
...
//---------------------------------------------------------------------
	...
	def receive_objects(structSet:List[String]) {
		...
		if(conditionStruct=="null") //inconsistent use of "" and "null" in <.struct> file
			conditionStruct=""
		//println("CardSet recieveObjects()  conditionStruct="+conditionStruct)
		
		//setCondition(conditionStruct)  
	//	go_continue=in.next //not operational
		}
