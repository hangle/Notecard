...
//-------------------------------------------------------------------
	...
	def executeNotecardChildren(obj:Any, notePanel:JPanel, taskGather:TaskGather, buttonSet:ButtonSet) { 
		obj match	{
									// CardSet executes a series of commands that
									// constitute a single Card set. 
			case cs:CardSet=> //
					println("Notecard:execute children")
			...
			case ft:NotecardTask=> //println("is NotecardTask")
			...
			case nf:NextFile=> //println("is NextFile")
	...
	def waitOverDoButtons(taskGather:TaskGather) {
		...
		if(buttonSet.isPriorButtonActivated) {
			println("Notecard: priorButton Activated")
	...
	def createCardWindow {
		...
		println("Notecard:  frame_width="+frame_width+"   frame_height="+frame_height)
