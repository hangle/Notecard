...
//-------------------------------------------------------------------
	...
	val backupMechanism= new BackupMechanism("CardSet")
		...
		// Instaniated in 'executeNotecardChildren when CardSet button==1
	...
	var manageNotecard:Notecard= _
		...
		// Set 'true' by '+Add' button allowing the processing of 'AddCardSet(s)
		// in 'executeNotecardChildren  "acs:CardSet=>"
		// Set 'false' by 'Next' when 'isLastAddCardSet' is true.
	var activatedAddButton= false
		// CardSets and AddCardSets have different BackupMechanism objects.
		// values are either "CardSet" or "AddCardSet"
	var whichBackup=""
		// 	
	...
	def executeNotecardChildren(obj:Node, 
		...
		obj match	{
			...
			case acs:CardSet if(acs.isAddCardSet) =>
					...
					// AddCardSets fall thru (not processed) unless 'activatedAddButton' 
					// has been set 'true' in 'waitOverDoButtons' by '+' button.
				if(activatedAddButton) {
							// set 'true' in 'case cs:CardSet=>' so that 'waitOverDoButtons'
							// 'case "+Add" ' 
						if(isFirstAddCardSet){
							isFirstAddCardSet=false
							buttonSet.start()
							}
			...
			case cs:CardSet=> //println("cs:CardSet") //
				...
				if(cs.hasAddCardSet) {
					...
					addBackupMechanism= new BackupMechanism("AddCardSet")
						// In '+Add' button of 'waitOverDoButtons' 1st AddCardSet 
						// is handled differently
						// Note: 'case acs:CardSet=>' set this variable 'false'.
					isFirstAddCardSet=true
					}
	...
	def waitOverDoButtons(taskGather:TaskGather, 
						  ...
						  defaultFont: DefaultFont,
						  whichBackup:String,
						  isFirstAddCardSet:Boolean):Unit={ 
		buttonSet.selectedButton match {
				...
				case "prior" =>
						...
						if(whichBackup=="cardSet")
								doPriorButton(backupMechanism)
							else{ //--if else then whichBackup=="addCardSet"

									// processing Add sets so armAddButton
								buttonSet.armAddButton	
								doPriorButton(addBackupMechanism)
								}
				case "*" =>
						doAsteriskButton(taskGather)
				case "+" =>
							// set 'true' in 'executeNotecardChildren' when a CardSet
							// had a dependent AddCardSet. The first AddCardSet sets 
							// it false.
						if(isFirstAddCardSet) {
							println("Notecard: isFirstAddCardSet")
							}
						  else
						  	buttonSet.start()
						...
						if(cardSet.isLastAddCardSet){
							...
							restoreCurrentNode
								...
								//Allow AddCardSet(s) to be executed.
	...
	def doPriorButton(backupMechanism:BackupMechanism) {
